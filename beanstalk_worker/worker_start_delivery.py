...
def notify_build_failure(namespace, notify_email, logs):
    ...
    msg_details['msg'] = "Container build " + namespace + \
        " failed due to error in build or test steps. " + \
        "Pleae check attached logs"
...
def run_command(command):
    p = Popen(command, bufsize=0, shell=True, stdout=PIPE,
...
def start_delivery(job_details):
    try:
        ...
        command_login = (
            "oc login https://openshift:8443 -u test-admin -p test --config=" +
            config_path+"/node.kubeconfig --certificate-authority=" +
            config_path+"/ca.crt")
        out = run_command(command_login)
        debug_log(out)

        debug_log(" change project to the desired one")
        command_change_project = (
            "oc project "+name_space+" --config="+config_path +
            "/node.kubeconfig")
        ...
        command_start_build = (
            "oc --namespace "+name_space+" start-build delivery --config=" +
            config_path+"/node.kubeconfig")
        ...
        if build_details == "":
            logger.log(level=logging.CRITICAL,
                       msg="build could not be started as openshift is not "
                       "reachable")
            return 1
        ...
        status_command = (
            "oc get --namespace "+name_space+" build/"+build_details +
            " --config="+config_path+"/node.kubeconfig|grep -v STATUS")
        is_running = 1
        ...
        log_command = (
            "oc logs --namespace "+name_space+" build/"+build_details +
        ...
        if is_complete < 0:
            ...
            notify_build_failure(name_space, to_mail, logs)
            debug_log(
